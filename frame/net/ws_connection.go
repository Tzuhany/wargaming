	...
	for {
		...
		case message, ok := <-c.WriteChan:
			...
			if err := c.Conn.WriteMessage(websocket.BinaryMessage, message); err != nil {
				klog.Errorf("client[%s] write message err :%v", c.Cid, err)
			}
		case <-c.pingTicker.C:
			if err := c.Conn.SetWriteDeadline(time.Now().Add(writeWait)); err != nil {
				klog.Errorf("client[%s] ping SetWriteDeadline err :%v", c.Cid, err)
			}
			if err := c.Conn.WriteMessage(websocket.PingMessage, nil); err != nil {
				klog.Errorf("client[%s] ping  err :%v", c.Cid, err)
				c.Close()
			}
		}
	}
...
func (c *WsConnection) readMessage() {
	...
	if err := c.Conn.SetReadDeadline(time.Now().Add(pongWait)); err != nil {
		klog.Errorf("SetReadDeadline err:%v", err)
		return
	}
	for {
		...
		if messageType == websocket.BinaryMessage {
			...
			klog.Errorf("unsupported message type : %d", messageType)
		}
	}
}
