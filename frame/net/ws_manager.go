...
import (
	...
	"wargaming/frame/protocol"
	"wargaming/frame/remote"
)

var (
	websocketUpgrade = websocket.HertzUpgrader{
...
func (m *Manager) serveWS(_ context.Context, c *app.RequestContext) {
	if m.websocketUpgrade == nil {
		m.websocketUpgrade = &websocketUpgrade
	}

	m.websocketUpgrade.Upgrade(c, func(conn *websocket.Conn) {
...
func (m *Manager) decodeClientPack(body *MsgPack) {
	...
	if err != nil {
		klog.Errorf("decode message err:%v", err)
		return
	}
	if err := m.routeEvent(packet, body.Cid); err != nil {
		klog.Errorf("routeEvent err:%v", err)
	}
}
...
func (m *Manager) HandshakeHandler(packet *protocol.Packet, c Connection) error {
	...
	if err != nil {
		klog.Errorf("encode packet err:%v", err)
		return err
	}
...
func (m *Manager) HandshakeAckHandler(packet *protocol.Packet, c Connection) error {
	klog.Info("receiver handshake ack message...")
	return nil
}

func (m *Manager) HeartbeatHandler(packet *protocol.Packet, c Connection) error {
	klog.Info("receiver heartbeat message:%v", packet.Type)
	var res []byte
	data, _ := sonic.Marshal(res)
	buf, err := protocol.Encode(packet.Type, data)
	if err != nil {
		klog.Error("encode packet err:%v", err)
		return err
	}
	return c.SendMessage(buf)
}
...
func (m *Manager) MessageHandler(packet *protocol.Packet, c Connection) error {
	...
	klog.Info("receiver message body, type=%v, router=%v, data:%v",
	...
	if connectorConfig != nil {
		...
		if err != nil {
			klog.Error("remote send msg selectDst err:%v", err)
			return err
		}
		msg := &remote.Msg{
		...
		if err != nil {
			klog.Error("remote send msg err：%v", err)
			return err
		}
	}
	return nil
...
func (m *Manager) KickHandler(packet *protocol.Packet, c Connection) error {
	klog.Info("receiver kick  message...")
	return nil
}
...
func (m *Manager) remoteReadChanHandler() {
	for {
		...
		case body, ok := <-m.RemoteReadChan:
			if ok {
				klog.Info("sub nats msg:%v", string(body))
				var msg remote.Msg
				if err := sonic.Unmarshal(body, &msg); err != nil {
					klog.Error("nats remote message format err:%v", err)
					continue
				}
...
func (m *Manager) Response(msg *remote.Msg) {
	...
	if !ok {
		klog.Info("%s client down，uid=%s", msg.Cid, msg.Uid)
		return
	}
	buf, err := protocol.MessageEncode(msg.Body)
	if err != nil {
		klog.Error("Response MessageEncode err:%v", err)
		return
	}
	res, err := protocol.Encode(protocol.Data, buf)
	if err != nil {
		klog.Error("Response Encode err:%v", err)
		return
	}
	if msg.Body.Type == protocol.Push {
...
func (m *Manager) remotePushChanHandler() {
	for {
		...
		case body, ok := <-m.RemotePushChan:
			if ok {
				klog.Info("nats push message:%v", body)
