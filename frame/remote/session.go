...
func (s *Session) pushChanRead() {
	for {
		...
		case data := <-s.pushChan:
			...
			result, _ := sonic.Marshal(msg)
			klog.Infof("push message dst:%v", msg.Dst)
			err := s.client.SendMsg(msg.Dst, result)
			if err != nil {
				klog.Errorf("push message err:%v, msg=%v", err, msg)
			}
		}
	}
}
	...
	for {
		...
		case data := <-s.pushSessionChan:
			...
			if err := s.client.SendMsg(msg.Dst, res); err != nil {
				klog.Errorf("push session data err:%v", err)
			}
		}
	}
}
